...
//
			if (iter == 1000) 
			{
				cout << "jamma_eig: hqr2 has not converged" << endl;
				exit(0);
			}
//
...
public:
   ...
   */
   ...
   Eigenvalue(const TNT::Array2D<Real> &B) {
	  TNT::Array2D<Real> A(B);
      n = A.dim2();
      V = Array2D<Real>(n,n);
      d = Array1D<Real>(n);
      e = Array1D<Real>(n);
      ...
      sksymmetric = 1;
      for (int j = 0; (j < n) && sksymmetric; j++) {
         for (int i = 0; (i < n) && sksymmetric; i++) {
            sksymmetric = (A[i][j] == -A[j][i]);
         }
      }
